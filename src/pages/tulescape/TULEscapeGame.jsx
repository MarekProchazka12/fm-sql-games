...
export default function TULEscapeGame() {
    ...
    const toggleOverlay = (type) => { setActiveOverlay(type); };
    ...
    const logQuery = async (queryData) => {
        const {data, error} = await supabase.from('query_logs').insert([{ 
    ...
    return (
        <div className="tul-page-container">
            <div className="tul-side-toolbar">
                <button className="tool-square" onClick={() => toggleOverlay('table')}>📊</button>
                <button className="tool-square" onClick={() => toggleOverlay('schema')}>📜</button>
                <button className="tul-tool-square" onClick={() => toggleOverlay('hint')}>💡</button>
            </div>
            ...
            <div className="tul-game-screen" style={{ backgroundImage: `url("/pageAssets/TULEscape/scenes/${currSceneData.img}")` }}>
                <div className="tul-info-bar">
                    <span>Uživatel: ladislav.jira</span> | <span>Status: INFILTRATED</span>
                </div>

                {currentScene > 1 && <button className="tul-previous-scene" onClick={() => setCurrentScene(prev => prev - 1)}>◀</button>}
                {currentScene <= lastSuccessScene && <button className="tul-next-scene" onClick={() => setCurrentScene(prev => prev + 1)}>▶</button>}
            
                <div className="tul-task-book">
                    <h3>MISE {currSceneData.id}</h3>
                    <p>{currSceneData.story}</p>
                    <p className="tul-prompt"><small>&gt; {currSceneData.prompt}</small></p>
                </div>

                <div className="tul-bottom-area">
                    <textarea spellCheck="false" className="tul-input-area" value={query} onChange={(e) => setQuery(e.target.value)} />
                    <button onClick={runSql} className="tul-send-btn">PROVÉST DOTAZ</button>
                </div>
